...
function submit_data (){
    if (validate_data()) {
    fetch("ControllerServlet/?" + "x_absolute=" + x_absolute + "&y_absolute=" + y_absolute + "&requestType=get_from_DB")
        ...
        .then(responseText => {
            ...
            fetch("/php/script.php/?" + "x_absolute=" + x_absolute + "&y_absolute=" + y_absolute + "&requestType=write_into_DB")
...
function checkAndUpload () {
    setTimeout( () => {
        if (bigData.length === 0){
            fetch("/php/tradingDots.php" + "?leftLimit=" + leftLimit + "&rightLimit=" + rightLimit + "&requestType=checkLimits")
                .then(response => response.text())
                .then(responseJson => {
